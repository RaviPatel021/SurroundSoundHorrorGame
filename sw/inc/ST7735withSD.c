...
void ST7735_DrawBitmap(int16_t x, int16_t y, const uint16_t *image, int16_t w, int16_t h, uint8_t swapColor){
  ...
  for(y=0; y<h; y=y+1){
    for(x=0; x<w; x=x+1){
			if(swapColor) pixelColor = ST7735_SwapColor(image[i]);
			else pixelColor = image[i];
                                        // send the top 8 bits
      writedata((uint8_t)(pixelColor >> 8));
                                        // send the bottom 8 bits
      writedata((uint8_t)pixelColor);
