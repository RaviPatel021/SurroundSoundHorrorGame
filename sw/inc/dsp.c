...
int32_t NoiseReject(int32_t x){
	// t2 and t1 come from the previous 2 function calls to NoiseReject()
	// use a history of samples to smooth out noise
  t2 = t1; 
  t1 = t0; 
  t0 = x/32; 
	
	// Rxx2 changes every sample based on past samples
  Rxx2 = ((K-1)*Rxx2 + t0*t2)/K;
	
	// bound within a range of -M to M
  if(Rxx2 < -M) Rxx2 = -M; 
  if(Rxx2 >  M) Rxx2 =  M;
	
	// scale x by the ratio Rxx2 / M, which ranges from -1.0 to 1.0
  return (Rxx2*x)/M; 
}
